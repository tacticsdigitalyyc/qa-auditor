...
export default function ScanPage() {
  ...
  const summaryForTab = (label) => {
    if (!report) return null
    const s = label === 'a' ? report.json_report?.summary?.a : report.json_report?.summary?.b
    return s
  }

  const issuesForTab = (label) => {
    if (!report) return []
    const r = label === 'a' ? report.json_report?.results?.a : report.json_report?.results?.b
    return r?.issues || []
  }

  const seoForTab = (label) => {
    if (!report) return null
    return label === 'a' ? report.seo_a : report.seo_b
  }

  const screenshotForTab = (label) => {
    if (!report) return null
    return label === 'a' ? report.screenshot_a : report.screenshot_b
  }
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-8">
        <Link to="/" className="text-gray-500 hover:text-gray-300 text-sm transition-colors">← New scan</Link>
            ...
            return (
              <div key={t} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 mb-4">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <a href={url} target="_blank" rel="noopener noreferrer" className="text-blue-400 text-sm hover:underline truncate">{url}</a>
                </div>
                  ...
                  <div className="flex gap-3 flex-wrap mb-5">
                    {SEVERITIES.map((sev) => (
                      <div key={sev} className="flex items-center gap-1.5 bg-gray-800 rounded-lg px-3 py-1.5">
                        ...
                        <span className="text-white font-bold text-lg">{s.bySeverity?.[sev] ?? 0}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="mb-5">
                  ...
                  <SeoTable
                    seoA={seoForTab('a')}
                    seoB={scan?.url_b ? seoForTab('b') : null}
                  />
                </div>
                ...
                <div>
                  <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                    Issues ({issuesForTab(t).length})
                  </h3>
                  <IssuesTable issues={issuesForTab(t)} />
                </div>
              </div>
            )
          })}
